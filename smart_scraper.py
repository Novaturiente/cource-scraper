...
def get_driver():
    ...
    service = ChromeService(ChromeDriverManager().install(), service_args=["--verbose", f"--log-path={log_path}"])
...
def login_if_needed(driver):
    ...
    try:
        ...
        try:
            WebDriverWait(driver, 5).until(EC.presence_of_element_located((By.ID, "email"))).send_keys("canadaeec@gmail.com")
            WebDriverWait(driver, 5).until(EC.presence_of_element_located((By.ID, "password"))).send_keys("EEC@baroda123")
            WebDriverWait(driver, 5).until(EC.element_to_be_clickable((By.ID, "btn-login"))).click()
...
def setup_search_page(driver, country_name):
    ...
    # Advanced Search
    WebDriverWait(driver, 10).until(EC.element_to_be_clickable((By.XPATH, "/html/body/div[2]/section/div/div[2]/div[1]/div/form/div[1]/div/div/div[3]/a"))).click()
    time.sleep(2)
    
    # Country
    WebDriverWait(driver, 10).until(EC.element_to_be_clickable((By.XPATH, "/html/body/div[2]/section/div/div[2]/div[1]/div/form/div[1]/div/div/div[4]/div/div[2]/div/div/div[2]/div/div[1]/div[1]/div"))).click()
    country_input = WebDriverWait(driver, 10).until(EC.presence_of_element_located((By.XPATH, "/html/body/div[2]/section/div/div[2]/div[1]/div/form/div[1]/div/div/div[4]/div/div[2]/div/div/div[2]/div/div[1]/div[1]/div/div[2]/div/div[1]/input")))
    ...
    # Year (2026)
    dropdown_button = WebDriverWait(driver, 10).until(EC.element_to_be_clickable((By.CSS_SELECTOR, 'button[data-id="Year"]')))
    dropdown_button.click()
    time.sleep(1)
    year_options = WebDriverWait(driver, 10).until(EC.presence_of_all_elements_located((By.CSS_SELECTOR, "div.dropdown-menu ul.dropdown-menu.inner.show li")))
...
def wait_for_overlay(driver):
    try:
        WebDriverWait(driver, 2).until(EC.visibility_of_element_located((By.CLASS_NAME, "sweet-overlay")))
        WebDriverWait(driver, 30).until(EC.invisibility_of_element_located((By.CLASS_NAME, "sweet-overlay")))
        time.sleep(1)
    except:
        time.sleep(2)
...
def scrape_base_courses(driver, level_name, csv_path, progress, page_task):
    ...
    try:
        ...
        progress.update(page_task, total=total_pages, completed=0, description=f"Base Scraping ({level_name})")
        
        fieldnames = ["Program Level", "Area of Study", "Special Requirements", "Course", "Course URL", "University", "Duration", "Open Semesters", "Closed Semesters", "Speciality", "Rankings", "Yearly Tuition Fee", "Application Fee"]
        ...
        while current_page <= total_pages:
            ...
            for article in articles:
                try:
                    ...
                    try:
                        uni_div = article.find_element(By.CSS_SELECTOR, "div[class*='universityProgram']")
                        try:
                            data["University"] = uni_div.find_element(By.CSS_SELECTOR, "div.con_text").text.strip()
                        except:
                    ...
                    data["Speciality"] = ""
                    try:
                        badges = [s.text.strip() for s in article.find_elements(By.CSS_SELECTOR, "div.highlight-badge-wrap > span")]
                    ...
                    data["Rankings"] = ""
                    try:
                        ranks = [r.text.strip().replace("\n", " ") for r in article.find_elements(By.CSS_SELECTOR, "div.rankborder.sp-rankborder")]
                    ...
                    for i, line in enumerate(lines):
                        if "Duration:" in line and i+1 < len(lines): data["Duration"] = lines[i+1]
                        if "Yearly Tuition Fee:" in line: data["Yearly Tuition Fee"] = line.replace("Yearly Tuition Fee:", "").strip()
                        if "Application Fee:" in line: data["Application Fee"] = line.replace("Application Fee:", "").strip()

                    # Intakes
                    data["Open Semesters"] = ""
                    data["Closed Semesters"] = ""
                    try:
                        open_s = [s.text.strip() for s in article.find_elements(By.CSS_SELECTOR, "div.openIntakeDiv span") if s.text.strip()]
                        data["Open Semesters"] = ", ".join(open_s[1:]) # Skip label
                    except: pass
                    try:
                        closed_s = [s.text.strip() for s in article.find_elements(By.CSS_SELECTOR, "div.closedIntakeDiv span") if s.text.strip()]
...
def tag_courses(driver, level_name, csv_path, status_table):
    ...
    for req_name in REQUIREMENTS:
        ...
        try:
            req_checkbox = WebDriverWait(driver, 5).until(EC.presence_of_element_located((By.XPATH, req_checkbox_xpath)))
            ...
            # Check results
            try:
                res_el = WebDriverWait(driver, 5).until(EC.presence_of_element_located((By.XPATH, RESULTS_SECTION_XPATH)))
                if "No Record Found" in res_el.text:
                    # Deselect and continue
                    if req_checkbox.is_selected(): driver.execute_script("arguments[0].click();", req_checkbox)
                    continue
                    
                # Iterate pages (lightweight scraping)
                pages_match = re.search(r"Page\s+\d+\s+of\s+(\d+)", res_el.text, re.IGNORECASE)
                total_pages = int(pages_match.group(1)) if pages_match else 1
                ...
                while curr <= total_pages:
                    ...
                    for article in articles:
                        try:
                            c_name = article.find_element(By.CSS_SELECTOR, "h3 a").text.strip()
                            try:
                                u_name = article.find_element(By.CSS_SELECTOR, "div[class*='universityProgram'] div.con_text").text.strip()
                            except:
                                u_name = article.find_element(By.CSS_SELECTOR, "div[class*='universityProgram']").text.strip()
                            ...
                            if key in courses_db:
                                ...
                                if req_name not in current_req:
                                    row["Special Requirements"] = f"{current_req}, {req_name}" if current_req else req_name
                    ...
                    if curr < total_pages:
                        try:
                            ...
                            driver.execute_script("arguments[0].scrollIntoView({behavior: 'smooth', block: 'center'});", nxt)
    ...
    for sa_name in STUDY_AREAS:
        ...
        try:
            # Open Dropdown
            btn = WebDriverWait(driver, 10).until(EC.element_to_be_clickable((By.CSS_SELECTOR, STUDY_AREA_BUTTON_SELECTOR)))
            driver.execute_script("arguments[0].scrollIntoView({behavior: 'smooth', block: 'center'});", btn)
            time.sleep(1)
            btn.click()
            time.sleep(1)
            
            inp = WebDriverWait(driver, 5).until(EC.presence_of_element_located((By.XPATH, STUDY_AREA_SEARCH_INPUT_XPATH)))
                            ...
                            try:
                                ...
                                if key in courses_db:
                                    ...
                                    if sa_name not in current_sa:
                                        row["Area of Study"] = f"{current_sa}, {sa_name}" if current_sa else sa_name
...
def worker(country_name, selected_levels, layout, progress, page_task, status_table):
    ...
    try:
        ...
        for level in selected_levels:
            ...
            csv_filename = f"{country_name.replace(' ', '_')}-smart.csv"
...
def main():
    ...
    selection = Prompt.ask("\n[bold green]Select Program Levels (comma-separated indices) or 'all'[/bold green]", default="all")
    ...
    t = threading.Thread(target=worker, args=(country_name, selected_levels, layout, progress, page_task, status_table_data))
    t.start()

    with Live(layout, refresh_per_second=4, screen=True, console=console) as live:
        while t.is_alive():
            layout["header"].update(Panel(Group(generate_status_table(), progress), title="Dashboard", border_style="blue"))
