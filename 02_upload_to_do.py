...
def upload_to_spaces(file_path, subfolder_name=None):
    ...
    if subfolder_name:
        object_name = f"{DO_SPACES_FOLDER}/{subfolder_name}/{file_name}"
    else:
...
def main():
    ...
    if selected_file:
        subfolder = Prompt.ask("[bold yellow]Enter subfolder name (leave empty for root)[/bold yellow]", default="")
        
        if Prompt.ask(f"Are you sure you want to upload [bold magenta]{selected_file}[/bold magenta]?", choices=["y", "n"], default="y") == "y":
            upload_to_spaces(selected_file, subfolder if subfolder else None)
        else:
